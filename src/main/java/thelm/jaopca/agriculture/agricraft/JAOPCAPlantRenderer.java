...
public class JAOPCAPlantRenderer {
	...
	public static void renderPlant(ITessellator tessellator, List<Pair<String, Boolean>> iconLocations, EnumRenderType renderType, int color) {
		List<Pair<TextureAtlasSprite, Boolean>> icons = iconLocations.stream().
				map(pair->Pair.of(tessellator.getIcon(new ResourceLocation(pair.getLeft())), pair.getRight())).
		...
		case HASH:
			for(Pair<TextureAtlasSprite, Boolean> pair : icons) {
				if(pair.getLeft() != null) {
					renderHashTagPattern(tessellator, pair.getLeft(), pair.getRight() ? color : 0xFFFFFF);
				}
			}
			break;
		}
